...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const wo of workOrders) {
      ...
      if (totalInstallments > 0 && wo.advanceReceived > 0) {
        const newAdvance = Math.max(0, wo.advanceReceived - totalInstallments)
        const remainingAmount = wo.finalPrice
          ? Math.max(0, wo.finalPrice - newAdvance - totalInstallments)
          : null
